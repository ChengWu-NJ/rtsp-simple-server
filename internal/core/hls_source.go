	...
	for {
		...
		if si.videoPID != nil && data.PID == *si.videoPID {
			...
			if data.PES.Header.OptionalHeader.PTSDTSIndicator == astits.PTSDTSIndicatorBothPresent {
				dts = time.Duration(float64(data.PES.Header.OptionalHeader.DTS.Base)) * time.Second / 90000
			} else {
				dts = pts
			}
			...
			if !si.videoInitialized {
				si.videoInitialized = true
				si.videoStartPTS = pts
				si.videoStartDTS = dts
				si.videoStartRTC = now
			}

			pts -= si.videoStartPTS
			dts -= si.videoStartDTS

			elapsed := now.Sub(si.videoStartRTC)
			if dts > elapsed {
				select {
				case <-si.ctx.Done():
					return fmt.Errorf("terminated")
			...
			if !si.audioInitialized {
				si.audioInitialized = true
				si.audioStartPTS = pts
			}

			pts -= si.audioStartPTS
			...
			si.audioEncoder.Encode(aus, pts)
		}
	}
}
